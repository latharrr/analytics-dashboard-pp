...
- `GOOGLE_MAPS_API_KEY`: optional. Powers the "New User Locations" page/Telegram button's reverse geocoding (signup coordinate → city/state). Create a key in Google Cloud Console with the Geocoding API enabled (requires billing enabled on that project). Without it, locations show as "Unknown location" — nothing else breaks.
