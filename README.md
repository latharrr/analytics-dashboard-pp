...
| `038_new_user_activity_per_user.sql` | Adds `analytics_new_user_activity_by_user(days_back, user_limit)`: one row **per active user** with their events aggregated into a jsonb array, instead of one row per event. The per-event `analytics_new_user_activity_detail()` routinely exceeds PostgREST's 1000-row response cap, and paging past it re-ran an expensive 7-way UNION once per page (30d ≈ 47s → timeout / blank page), while a single un-paged call showed only ~13-21 power users instead of the 320+ who were active. The per-user shape is ≤`user_limit` rows (one DB execution, ~3s) and preserves every user's complete activity; `getNewUserActivityDetail()` calls it and flattens back to the per-event shape the table/CSV already use. |
| `039_verified_users_verification_filter.sql` | Adds a `verification_filter` param (`'both'` default / `'digilocker'` / `'college'` / `'either'`) to `analytics_verified_users_detail()`, so Verified Users can show/export Digilocker-only or college-only verified users instead of always requiring both. Signature change, so the old 5-arg function is dropped first (matches the pattern in migrations 023/028/030) rather than `CREATE OR REPLACE`d, to avoid an ambiguous overload. Output columns are unchanged — `digilocker_verified_at`/`college_verified_at` already carried a per-method timestamp; the dashboard now renders both as separate badge columns. Backs the new "Verified via" filter and Digilocker/College columns on the Verified Users page, plus its new `.xlsx` export. |
| `040_user_activity_detail.sql` | Adds `analytics_user_activity_detail(target_user)`: one user's **full** tracked-activity timeline (all-time) — the same 7-way UNION as `analytics_all_users_detail`'s LATERAL but without the `LIMIT 1`, with the richer pool-name detail. Powers expand-on-demand rows on **All Users** and on New User Activity's "All users" mode (`GET /api/user-activity/[userId]`). Per-user perf: filters the big event tables in `public.*` by the user column (uses the migration-032 indexes) then `DISTINCT ON (id)` to dedup; small label tables join via `dedup.*`. |
| `041_all_users_and_new_user_activity_inactive.sql` | Makes **inactive users visible**. `analytics_all_users_detail` gains `activity_filter` (all/active/inactive, based on the normalised `last_active`). `analytics_new_user_activity_by_user` now **LEFT-joins** activity onto the signup cohort, so new users who did nothing are returned too (activity_count 0, events `[]`) instead of being hidden (~329 of ~650 7-day signups were invisible); it also takes `activity_filter` and orders active users first. Both change arity, so the old signatures are `DROP`ped first. |
| `042_direct_chats.sql` | Adds the **Direct Chats** browser: `analytics_chat_conversations` (one row per room — DM or pool — with participants, message count, first/last message), `analytics_chat_room_messages` (a room's message timeline with sender + DM recipient + text), `analytics_chat_users` (one row per human sender with DM/pool/total counts), and `analytics_chat_user_messages` (a user's sent messages with room/counterpart + text). Grouped counts aggregate `dedup.chat_messages` in SQL (no 3× inflation, no JS-aggregation cap); single-room/single-user lists filter `public.chat_messages` then `DISTINCT ON (id)` to keep indexes usable. Adds indexes on `chat_messages(room_id, created_at)` and `chat_members(room_id)` / `(user_id)`. |
| `043_new_user_locations_all_users.sql` | `analytics_new_user_locations_detail` now treats `days_back <= 0` as **all users** (skip the signup window), backing the "All users" toggle on the New User Locations page. Backward-compatible (`CREATE OR REPLACE`, positive values unchanged). |
| `044_engagement_scores_and_chat_user_rooms.sql` | Adds `analytics_all_users_engagement(...)` — the **All Users** page's new backing function, adding **engagement density** (`total_activities ÷ active_days`) and **retention score** (`active_days ÷ days_since_signup`, i.e. fraction of lifetime active) alongside `total_activities` and `active_days` (distinct calendar days with any tracked event), all globally sortable (new sort keys `activities`, `engagement_density`, `retention_score`). A single grouped scan of the `dedup.*` activity tables — the per-user activity count deduped from `public.pg_hunt_queries` inline (no `dedup.pg_hunt_queries` view exists, so raw ~3× rows would otherwise triple-count PG searches). `page_size` clamp is **10,000** (not 200) so the CSV/XLSX export pages the whole filtered set instead of truncating to 200. Also adds `analytics_chat_user_rooms(target_user)` for the Direct Chats "by user" drill-down (the rooms/pools a user messaged in). Additive: leaves the 041 `analytics_all_users_detail` in place; the app switches to the new function. |
| `045_engagement_dedup_sort_indexes.sql` | Adds plain non-unique indexes on `(id)` / `(id, updated_at DESC)` matching each dedup view's `ORDER BY` (`chat_messages`, `pools`, `trust_ledger`, `pool_participants`, `pool_flat`, `pool_flatmate`, `pg_hunt_queries`) — same safe pattern as 032, no PK/unique constraint. Migration 032 had indexed these same tables only on their *filter* columns for per-row LATERAL lookups, not the `(id)` sort key the 044 aggregate scans on. **Necessary but not sufficient**: this was applied while chasing the `/api/all-users` timeout and did not fix it — see 046 for the actual fix. Kept because it does speed up the dedup sorts generally, including 046's nightly refresh. |
| `046_user_engagement_materialized.sql` | **Fixes All Users returning 0 users.** 044's per-user aggregate scans ~1.27M raw rows across 7 tables on every page load. Measured on the live DB: the deployed function takes **8,991 ms cold** / ~0.7 s warm, against an **8 s** budget (PostgREST connects as `authenticator`, which sets `statement_timeout=8s`; `service_role` has no rolconfig so it inherits) — so every cold visit times out, which is every visit in practice. The SQL editor runs as `postgres` with no such limit, which is why 044 looked fine when written. Fix: precompute the aggregate into `mv_user_engagement` (user_id, total_activities, active_days) with a unique index on `user_id`, add it to the existing 21:30 `pg_cron` refresh job (same re-register-in-place approach as 017), and `CREATE OR REPLACE` the function to `LEFT JOIN` the view instead of computing `agg` inline — signature unchanged, so no app code change. The view reuses 044's aggregate expression verbatim (same `dedup.*` views) rather than a faster narrow rewrite, since refresh runs as `postgres` where its ~20 s cost is irrelevant and the narrow form would change dedup semantics. **Removed ~19 s of CPU but did not fix the timeout** — measured after applying, the function still ran **14,248 ms cold** (0.9 s warm), because the residual cost is I/O, not CPU. Superseded by 047, which drops `mv_user_engagement`. Also corrects 044's sizing comment: `chat_messages` is 520,243 raw rows → 32,006 deduped (~16× duplication), not the "~71k events" assumed. |
| `047_all_users_roster_materialized.sql` | **The fix that actually lands.** After 046, `EXPLAIN (ANALYZE, BUFFERS)` showed the remaining cost was cold random I/O: the roster reads **102,246 raw `public.users` rows** through the index to dedup down to 7,516, and the per-page LATERAL does 50 users × 7 tables = **350 scattered reads** across `chat_messages` (520k), `trust_ledger` (477k), `pool_participants` (222k). Warm that's ~0.9 s; cold it's ~14 s, and on this instance (free tier, flagged EXCEEDING USAGE LIMITS) a page visited a few times a day is cold every time. Rather than shave the margin a third time, this precomputes the **entire roster** — every column the page renders, for every non-bot user — into `mv_all_users_roster`: **7,516 rows / 890 kB**, so even a fully cold read is a few sequential pages. The request-time function becomes filter + sort + paginate over that, touching no big table and doing no LATERAL. Verified before shipping: `total_activities`/`active_days` match 044's definition for **all 1,710** users with activity (0 mismatches, 0 users present in only one), and the `DISTINCT ON` last-activity matched the live 7-way LATERAL on 25/25 sampled users. `pool_participants → pools` is a **LEFT** JOIN on purpose — 044 used no join there, so an INNER JOIN would silently undercount. Drops the now-subsumed `mv_user_engagement` and swaps the nightly job to refresh the roster (~35 s, non-CONCURRENTLY, matching 017's deliberate choice). |

| `048_all_users_category_intent.sql` | Adds **Category Intent** to All Users: a `Top category` column (the content category each user views most, with that category's view count), a category filter, and a `category_views` sort key. Source is `user_tag_affinity` (one row per user+tag with `view_count`, `join_count`, `total_dwell_ms`, `score`) joined `tags → tag_categories`, rolled up to 11 categories. Measured for non-bot users: 38,971 rows / **1,735 users** / 120,977 views / ~462 h dwell, written daily. Top category is ranked by **views** (the question is who is *looking* at what); the app's own `score` agrees on 1,471/1,735 (85%) and is used only as tie-break, then label, so the pick is deterministic across refreshes. Folded into `mv_all_users_roster` (+2.1 s on its ~35 s nightly build) as `top_category`, `top_category_views`, `top_category_dwell_ms`, `category_count`, so the page cost is unchanged. Adds `analytics_all_users_categories()` for the filter dropdown. Signature change (new `category_filter` param), so the 10-arg function is `DROP`ped first per 023/028/030/039. Caveats: coverage is ~23% of non-bot users (NULL = no signal, **not** zero interest, rendered "—"), and `chat_msg_count`/`dismiss_count` are 0 on every row so nothing uses them. Self-contained — rebuilds the roster and re-registers the cron job, so it lands correctly whether or not 047 ran. |

| `049_category_intent_dashboard.sql` | Adds the **Category Intent** page (`/category-intent`): the platform-wide view of what users look at, one row per tag category with users, views, joins, likes, dwell, views-per-user, the 3 most-viewed tags inside it, and last signal. Backed by `mv_category_intent` (11 rows) + `analytics_category_intent()`, added to the nightly refresh. Materialized for the same reason as 046–048 — request-time queries here are fine warm and blow the 8 s budget cold. **Counts intentionally differ from the All Users category filter**: this aggregates *all* affinity rows, so a user engaging with 11 categories is counted in all 11, whereas `mv_all_users_roster` keeps only each user's single top category. Both are correct for their own question; don't expect them to reconcile. Each row links to `/all-users?category=<label>`, which seeds the All Users category filter from the URL. |

| `050_pool_category_intent.sql` | Adds a **Pool categories** tab to Category Intent, from `pools.category` — the app's actual verticals (Ask Around, Ranting, Buy & Sell, Event, Cab Share, Flat, Flatmate, PG) with pools created, distinct creators, joins and joiners, plus an "All Categories" selector. A **separate dimension** from 049's content/tag categories, hence a separate tab: 049 is what users *browse* (views/dwell from tag affinity), this is what they *create and join*. There is no view or dwell data for pools, so merging them would put unrelated numbers in shared columns. Backed by `mv_pool_category_intent` (8 rows) + `analytics_pool_category_intent()`, added to the nightly refresh; labels are mapped in SQL so the page and API agree. Two data facts baked into the UI: **flat/flatmate/pg have zero participants** — they're listing-type pools nobody joins, so the table shows "n/a" rather than a misleading 0 — and **`ranting` is included** despite not being in the original request, because it is the largest vertical by pools (1,473) and omitting it would distort every total. |

| `051_pool_category_users.sql` | Makes the Pool categories rows **clickable**: expand a category to see the users behind it — name, phone, pools created, pools joined, last active in that category, trust. Backed by `mv_pool_category_users` (one row per category+user where the user created *or* joined there; 2,441 rows / 656 kB) + `analytics_pool_category_users(category, row_limit)`, capped at 500 rows per category and added to the nightly refresh. Materialized rather than live because the equivalent join over `pools` + `pool_participants` measured **1.2–2.0 s warm** per category, which the cold multiple would push near the 8 s budget — the same trap 044–047 fell into. A user active in several categories appears once per category, by design. |
