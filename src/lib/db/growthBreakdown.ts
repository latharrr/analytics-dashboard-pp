...
/** Top colleges by verified user_colleges membership. Aggregated in JS (user_colleges is a small table). The raw table holds duplicate import snapshots, so membership is deduplicated by (user_id, college_id) rather than counted per row. */
export async function getTopCollegesByUsers(limit = 5): Promise<BarDatum[]> {
  ...
  const { data: userColleges, error: ucError } = await supabase
    .from("user_colleges")
    .select("college_id, user_id");
  if (ucError || !userColleges || userColleges.length === 0) return [];
