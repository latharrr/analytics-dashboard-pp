...
/** Top colleges by verified user_colleges membership. Aggregated in JS (user_colleges is a small table, ~150 rows). */
export async function getTopCollegesByUsers(limit = 5): Promise<BarDatum[]> {
  ...
  for (const row of userColleges as { college_id: string }[]) {
